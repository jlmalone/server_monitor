...
protection-status row, a **Worker** start/stop control, and a **Transfers**
panel (active file transfers — %, rate, ETA — from a queue CLI's JSON, across one or
more machines; failed rows get a one-click **Resume** when an optional `runCommand`
is configured, otherwise the panel is read-only). An unresolved failed transfer also
downgrades the menu-bar tint off green. Their behavior is generic; all machine-specific
