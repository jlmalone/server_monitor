...
The optional **VPN** and **Worker** menu-bar panels read machine-specific values
(host paths, scripts, log locations) from an **untracked** local config at
`~/.config/server-monitor/worker.json` — see [`config/worker.example.json`](./config/worker.example.json)
for the schema and [CONFIG.md](./CONFIG.md) for details. These files are never
committed; they hold per-machine values and are synced across your own machines
out-of-band, **not** via git. With no config present, the panels stay generic and inert.
