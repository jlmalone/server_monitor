...
class ServiceMonitor: ObservableObject {
    ...
    private var identifierPrefix: String = "vision.salient"
    ...
    func checkAllServices() {
        ...
        Task.detached {
            var results: [(identifier: String, status: ServiceStatus, pid: Int?)] = []
            for service in snapshot {
                if service.enabled == false {
                    results.append((service.identifier, .unknown, nil))
                } else {
                    let result = await Self.checkService(service)
                    results.append((service.identifier, result.0, result.1))
                }
            }
            ...
            await MainActor.run { [weak self] in
                ...
                for result in finalResults {
                    guard let index = self.services.firstIndex(where: { $0.identifier == result.identifier }) else { continue }
                    let service = self.services[index]
                    if self.lastStatuses[result.identifier] == .running && result.status == .stopped {
                        self.sendNotification(serviceName: service.name)
                    }
                    self.lastStatuses[result.identifier] = result.status
                    self.services[index].status = result.status
                    self.services[index].pid = result.pid
    ...
    private nonisolated static func checkService(_ service: Service) async -> (ServiceStatus, Int?) {
        let launchd = launchdStatus(service.identifier)
        guard launchd.0 == .running else { return launchd }

        if let value = service.healthCheckURL, let url = URL(string: value) {
            var request = URLRequest(url: url)
            request.timeoutInterval = 5
            do {
                let (_, response) = try await URLSession.shared.data(for: request)
                if let http = response as? HTTPURLResponse, !(200..<400).contains(http.statusCode) {
                    return (.stopped, launchd.1)
                }
            } catch {
                return (.stopped, launchd.1)
            }
    ...
    private nonisolated static func launchdStatus(_ identifier: String) -> (ServiceStatus, Int?) {
        let target = "gui/\(getuid())/\(identifier)"
        let result = ProcessRunner.run(["/bin/launchctl", "print", target], timeout: 5)
        guard result.succeeded else { return (.stopped, nil) }
        let output = result.output
        let pid = output.range(of: #"pid = (\d+)"#, options: .regularExpression).flatMap { range in
            Int(output[range].replacingOccurrences(of: "pid = ", with: ""))
        }
        let running = output.contains("state = running") || pid != nil
        return running ? (.running, pid) : (.stopped, nil)
    }
...
"""
    ...
    func startService(_ service: Service) {
        ...
        let (status, _) = Self.launchdStatus(service.identifier)
    ...
    func stopService(_ service: Service) {
        ...
        let (_, pid) = Self.launchdStatus(service.identifier)
    ...
    var overallStatus: ServiceStatus {
        let enabledServices = services.filter { $0.enabled != false }
        if enabledServices.allSatisfy({ $0.status == .running }) { return .running }
        else if enabledServices.contains(where: { $0.status == .stopped }) { return .stopped }
        return .unknown
    }
    
    var runningCount: Int {
        services.filter { $0.enabled != false && $0.status == .running }.count
    }
    
    var totalCount: Int {
        services.filter { $0.enabled != false }.count
    }
