...
final class WorkerStatusMonitor: ObservableObject {
    ...
    private func run(_ action: String) {
        ...
        busy = true
        Task.detached {
            let directory = URL(fileURLWithPath: c.repoDir)
            let script = directory.appendingPathComponent(c.script).path
            _ = ProcessRunner.run([script, action, c.arg], timeout: 60,
                                  includeStandardError: true, currentDirectory: directory)
            await MainActor.run {
                self.busy = false
                self.refresh()
            }
...
struct TransfersQueueReport: Codable {
    let schema: Int?
    let generatedAt: String?
    let queue: [TransfersQueueItem]
    let summary: TransfersQueueSummary
}
...
struct TransferRow: Identifiable {
    ...
    let title: String
    let status: String
    /// Human label for the second line when there is no live rate: "starting…"
    /// while a running entry has no byte total yet (mid-preflight or just re-attempted),
    /// otherwise the raw status. Keeps a live-but-pre-byte transfer from reading as frozen.
...
final class TransfersMonitor: ObservableObject {
    ...
    private var timer: Timer?
    /// Cadence when idle (nothing running/pending): cheap, from config, default 60s.
    private let idleInterval: TimeInterval
    /// Cadence while a transfer is active: fast enough that the panel tracks it
    /// instead of showing a minute-stale (often pre-byte) snapshot that looks frozen.
    private let activeInterval: TimeInterval

    init() {
        let cfg = TransfersConfig.load()
        self.config = cfg
        self.configured = (cfg != nil)
        self.idleInterval = cfg?.pollSeconds ?? 60
        self.activeInterval = min(cfg?.pollSeconds ?? 60, 10)
        guard cfg != nil else { return }
        refresh()   // reschedules the next tick on completion
    }

    deinit { timer?.invalidate() }

    /// One self-rescheduling tick: poll fast while a transfer is active, slow while
    /// idle, so an open menu tracks live progress without spawning the queue CLI
    /// every few seconds when nothing is running. Called after every refresh.
    ...
    var headlineColor: Color {
        if !configured { return .secondary }
        if failed > 0 || lastError != nil { return .orange }
    ...
    private nonisolated static func readSource(_ source: TransfersSource, timeout: TimeInterval) -> Data? {
        ...
        guard let command = source.command, !command.isEmpty else { return nil }
    ...
    private nonisolated static func toRow(_ i: TransfersQueueItem, machine: String) -> TransferRow {
        ...
        // A running entry with no byte total yet is mid-preflight (hashing/SSH checks)
...
final class TransferHistoryLoader: ObservableObject {
    ...
    /// Run argv directly with a deadline and bounded output.
    private nonisolated static func runArgv(_ argv: [String]) -> Data? {
        let result = ProcessRunner.run(argv, timeout: 60, includeStandardError: false,
