...
struct SettingsView: View {
    ...
    @ObservedObject var backgroundService: BackgroundServiceManager
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 12) {
                Image(systemName: backgroundService.agentStatus == .enabled
                      ? "checkmark.circle.fill" : "exclamationmark.circle.fill")
                    .foregroundColor(backgroundService.agentStatus == .enabled ? .green : .orange)
                VStack(alignment: .leading, spacing: 2) {
                    Text("Infrastructure Agent").font(.subheadline.bold())
                    Text("Agent: \(backgroundService.agentStatusText) · Login: \(backgroundService.loginStatusText)")
                        .font(.caption2)
                        .foregroundColor(.secondary)
                ...
                Spacer()
                if backgroundService.needsApproval {
                    Button("Open Login Items") { backgroundService.openLoginItemsSettings() }
                } else if backgroundService.agentStatus != .enabled {
                    Button("Register") { backgroundService.registerIfNeeded() }
                }
                Button(action: { backgroundService.refresh() }) {
...
struct SettingsView_Previews: PreviewProvider {
    static var previews: some View {
        SettingsView(monitor: ServiceMonitor(), backgroundService: BackgroundServiceManager(autoRegister: false))
    }
}
